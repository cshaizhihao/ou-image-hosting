...
<p align="center">
  <img src="https://img.shields.io/badge/version-0.9.0-ef8f8f" alt="Version 0.9.0" />
  <img src="https://img.shields.io/badge/progress-8%20%2F%2010-303030" alt="Round 8 of 10" />
...
- 多工作区与 Owner、Admin、Editor、Viewer 四级 RBAC 权限模型
- 团队邀请、角色调整、成员移除、工作区切换和所有权转移
- 具备八种精确 Scope、有效期、吊销和 IPv4/IPv6/CIDR 白名单的 API Token
- 严格 Bearer 认证、固定工作区、会话管理和来源校验
- TOTP 双因素认证、恢复代码、登录挑战与重放保护
- 安全、协作和系统三类站内通知、分类偏好、已读状态与免打扰
- 审计时间线、操作筛选和防表格公式注入的 CSV 导出
- schema v6 自动迁移，并兼容恢复 v5 完整备份
- 32 个自动化测试、生产构建与完整桌面、移动深色端到端验证
