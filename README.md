...
后台和公共上传默认分开。普通注册用户只能使用公共图床和查看自己的上传历史，不会误进后台。

公共上传可以配置速率限制、每日额度、人机验证、IP 封禁和审计记录。后台不会展示用户密码，只提供重置入口。
